...
// ── Layout constants (vertical 240 × 320) ────────────────────────
//
//  16  CLAUDE PLAN                 14:32 ●
//  32  SUBSCRIPTION
//
//  58  SESSION 5H
//  74  42%          (hero, centered)
...
// 206  19%
// 232  ██░░░░░░░░░░░░░░
// 250  OPUS 5%   SONNET 18%
//
// 272  ─── divider ───
//
// 284  EXTRA USAGE    $12.50 / $50 USD
// 302  ████░░░░░░░░░░░░░░░░
//
// 312  UPDATED 2M AGO
//
...
static constexpr int Y_SUB        = 32;

static constexpr int Y_SESS_LBL   = 58;
static constexpr int Y_SESS_HERO  = 74;
static constexpr int Y_SESS_BAR   = 134;
static constexpr int Y_SESS_RESET = 152;

static constexpr int Y_DIV1       = 176;

static constexpr int Y_WK_LBL     = 190;
static constexpr int Y_WK_PCT     = 206;
static constexpr int Y_WK_BAR     = 232;
static constexpr int Y_WK_SUB     = 250;

static constexpr int Y_DIV2       = 272;

static constexpr int Y_EX_LINE    = 284;
static constexpr int Y_EX_BAR     = 302;

static constexpr int Y_FOOTER     = 310;
...
void ScreenPlan::buildLayout() {
    ...
    lv_obj_set_pos(lbl_sub_, X_PAD, Y_SUB);
    ...
    lv_obj_set_pos(lbl_session_, X_PAD, Y_SESS_LBL);
    ...
    lv_obj_align(lbl_sess_pct_, LV_ALIGN_TOP_MID, 0, Y_SESS_HERO);

    bar_session_ = lv_bar_create(scr_);
    lv_obj_set_size(bar_session_, CONTENT_W, Sp::bar_h);
    lv_obj_set_pos(bar_session_, X_PAD, Y_SESS_BAR);
    ...
    lv_obj_set_pos(lbl_sess_reset_, X_PAD, Y_SESS_RESET);

    // ── Divider 1 ────────────────────────────────────────────────
    static lv_point_precise_t div1_pts[2] = {
        {(lv_value_precise_t)X_PAD, 0},
        {(lv_value_precise_t)(SCREEN_W - X_PAD), 0}
    };
    line_div1_ = lv_line_create(scr_);
    lv_line_set_points(line_div1_, div1_pts, 2);
    lv_obj_set_style_line_color(line_div1_, Color::hairline(), 0);
    lv_obj_set_style_line_width(line_div1_, Sp::hairline, 0);
    lv_obj_set_pos(line_div1_, 0, Y_DIV1);
    ...
    lv_obj_set_pos(lbl_weekly_, X_PAD, Y_WK_LBL);
    ...
    lv_obj_set_pos(lbl_week_pct_, X_PAD, Y_WK_PCT);

    bar_weekly_ = lv_bar_create(scr_);
    lv_obj_set_size(bar_weekly_, CONTENT_W, Sp::bar_h);
    lv_obj_set_pos(bar_weekly_, X_PAD, Y_WK_BAR);
    ...
    lv_obj_set_pos(lbl_week_sub_, X_PAD, Y_WK_SUB);

    // ── Divider 2 ────────────────────────────────────────────────
    static lv_point_precise_t div2_pts[2] = {
        {(lv_value_precise_t)X_PAD, 0},
        {(lv_value_precise_t)(SCREEN_W - X_PAD), 0}
    };
    line_div2_ = lv_line_create(scr_);
    lv_line_set_points(line_div2_, div2_pts, 2);
    lv_obj_set_style_line_color(line_div2_, Color::hairline(), 0);
    lv_obj_set_style_line_width(line_div2_, Sp::hairline, 0);
    lv_obj_set_pos(line_div2_, 0, Y_DIV2);
    ...
    lv_obj_set_pos(lbl_extra_lbl_, X_PAD, Y_EX_LINE);
    ...
    lv_obj_set_pos(lbl_extra_val_, X_PAD + 110, Y_EX_LINE + 3);

    bar_extra_ = lv_bar_create(scr_);
    lv_obj_set_size(bar_extra_, CONTENT_W, Sp::bar_h);
    lv_obj_set_pos(bar_extra_, X_PAD, Y_EX_BAR);
...
void ScreenPlan::update(const ClaudePlanSnapshot& snap) {
    // Session hero
    char sessBuf[8];
    ...
    // Weekly Opus/Sonnet sub-line (only show if at least one is non-zero)
    char subBuf[64];
    if (snap.weekly_opus_pct > 0.01f || snap.weekly_sonnet_pct > 0.01f) {
        if (snap.weekly_resets_at > 0) {
            ...
            char rbuf[12];
            strftime(rbuf, sizeof(rbuf), "%b %d", &lt);
            snprintf(subBuf, sizeof(subBuf),
                "OPUS %d%%  SONNET %d%%  RESETS %s",
    ...
    } else if (snap.weekly_resets_at > 0) {
        struct tm lt;
        localtime_r(&snap.weekly_resets_at, &lt);
        strftime(subBuf, sizeof(subBuf), "RESETS %b %d", &lt);
    } else {
        subBuf[0] = '\0';
    }
    ...
    // Footer — either the error hint or "updated X ago"
    char foot[48];
    if (snap.error != CLAUDE_PLAN_OK && snap.valid) {
        // We have cached data but the latest fetch failed.
...
void ScreenPlan::setStatusDot(lv_color_t c) {
